...
template Main() {
  ...
  signal input val_lb;
  signal input val_ub;
  signal output out_val_lb;
  signal output out_val_ub;
  ...
  component val_range_check = ScalarRangeChecker(248);
  val_range_check.in <== val;
  val_range_check.lower_bound <== val_lb;
  val_range_check.upper_bound <== val_ub;
  out_val_lb <== val_lb;
  out_val_ub <== val_ub;
}
